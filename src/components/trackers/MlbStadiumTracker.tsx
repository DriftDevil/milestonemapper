...
import { MlbBallparksMap } from '@/components/map/MlbBallparksMap';
          ...
          {stadiums.length > 0 && (
            <AlertDialog>
              <AlertDialogTrigger asChild>
                <Button variant="outline" size="sm" className="w-full sm:w-auto">Clear Visited Ballparks</Button>
              </AlertDialogTrigger>
              <AlertDialogContent>
                <AlertDialogHeader>
                  ...
                  {stadiums.length > 0 ? (
                    <p>
                      {!showVisited && searchTerm === ''
                        ? 'All MLB ballparks visited! Check "Show Visited" to see them.'
                        : 'No MLB ballparks found matching your criteria.'}
                    </p>
                  ) : (
                    <p>No MLB ballparks to display.</p>
                  )}
                </div>
              )}
            </div>
            ...
            <div className="w-full aspect-[16/10] bg-muted/20 rounded-md overflow-hidden border relative">
              ...
              {!hasCoordinateData && (
                <div className="absolute inset-0 flex items-center justify-center text-center text-muted-foreground p-4 bg-background/80">
                  <p>Map view requires latitude and longitude data for ballparks.<br/>The map will populate once this data is available.</p>
                </div>
              )}
            </div>
          ) : (
