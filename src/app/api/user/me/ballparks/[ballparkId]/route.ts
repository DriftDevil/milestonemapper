...
const CONTEXT = "API User Ballpark Management";
...
// POST handler to add a visited ballpark
export async function POST(request: NextRequest, { params }: { params: { ballparkId: string } }) {
    if (!EXTERNAL_API_URL) {
    ...
    const { ballparkId } = params;
    if (!ballparkId) {
        return NextResponse.json({ message: 'Ballpark ID is required.' }, { status: 400 });
    }
    ...
    const url = new URL(`/user/me/ballparks/${ballparkId}`, EXTERNAL_API_URL).toString();
...
// DELETE handler to remove a visited ballpark
export async function DELETE(request: NextRequest, { params }: { params: { ballparkId: string } }) {
    if (!EXTERNAL_API_URL) {
        logger.error(CONTEXT, 'EXTERNAL_API_BASE_URL is not set.');
        return NextResponse.json({ message: 'API endpoint not configured.' }, { status: 500 });
    }
