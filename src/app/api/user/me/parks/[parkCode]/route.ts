...
const CONTEXT = "API User Park Management";
...
// POST handler to add a visited park
export async function POST(request: NextRequest, { params }: { params: { parkCode: string } }) {
    if (!EXTERNAL_API_URL) {
...
// DELETE handler to remove a visited park
export async function DELETE(request: NextRequest, { params }: { params: { parkCode: string } }) {
    if (!EXTERNAL_API_URL) {
        logger.error(CONTEXT, 'EXTERNAL_API_BASE_URL is not set.');
        return NextResponse.json({ message: 'API endpoint not configured.' }, { status: 500 });
    }
